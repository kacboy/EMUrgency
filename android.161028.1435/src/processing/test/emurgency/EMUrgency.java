...
String directory = new String(Environment.getExternalStorageDirectory().getAbsolutePath()) + "/aFolderName";
String myfn="atextFilet.txt";   

PFont nokia;
PImage earth, moon, logo, airSmall, airBig, obstacle1, obstacleBig, meteor1, meteor2, meteor3, meteor4, top, top2, front, front2, front3, front4, packtop;
...
public void draw() { 
  ...
  println("gameState:"+gameState+" obstacles:"+obstacle.size(), "dodged:"+dodged, "obstacleMax:"+obstacleMax, "obstacleMax/2:"+obstacleMax/2);
}
...
public void HUD() {
  if (gameState==-1) {
    ...
    noStroke();
    rectMode(CORNER);
    if (Button("RESET", width/2-width/8, height-width/6, width/4, width/10)&&player.health<=0) {
      gameState=0;
      spawnPack=0;
      setup();
    }
    text("AIR:00.0 | TIME:"+minutes+":"+nf(seconds, 2), width/2, width/12);
  } else if (gameState==0) {
    //start menux
    logo();
    fill(255);
    ...
    rectMode(CORNER);
    if (Button("HELP", width/5-width/8, height-width/6, width/4, width/10)) {
      gameState=2;
      player.location.x=-100;
      player.location.y=-100;
    } else if (Button("SETTINGS", width/2-width/8, height-width/6, width/4, width/10)) {
      gameState=3;
      player.location.x=-100;
      player.location.y=-100;
    } else if (Button("CREDITS", width/5*4-width/8, height-width/6, width/4, width/10)) {
      gameState=4;
      player.location.x=-100;
      player.location.y=-100;
    }
    ...
    fill(255);
    textAlign(CENTER);
    text("HELP MENU", width/2, height/2);
    rectMode(CORNER);
    if (Button("BACK", width/2-width/8, height-width/6, width/4, width/10)) {
      gameState=0;
      setup();
    }
  } else if (gameState==3) {
    //settings
    fill(255);
    text("MUSIC | SOUNDS | RESET HIGH SCORE", width/2, height/2);
    minsHS=0;
    secsHS=0;
    rectMode(CORNER);
    if (Button("BACK", width/2-width/8, height-width/6, width/4, width/10)) {
  ...
  if (gameState!=-1&&gameState<2) {
    fill(255);
    if (player.health>=10) text("AIR:" +player.health+" | TIME:"+minutes+":"+nf(seconds, 2), width/2, width/12);
    else if (player.health<10&&player.health>0) text("AIR:0" +player.health+" | TIME:"+minutes+":"+nf(seconds, 2), width/2, width/12);
    else {
      text("AIR:00.0 | TIME:"+minutes+":"+nf(seconds, 2), width/2, width/12);
    }
  } 
  text("HS:"+minsHS+":"+nf(secsHS, 2), width/2, width/8);
}
...
public void credits() {
  logo();
  fill(255);
  text("VERSION 0."+((year()-2016)+(30*month()-8)+day()), width/2, width/12);
  text("MARCUS BLACKSTOCK 2016", width/2, height/2);
  textSize(width/25/2);
  text("EXTRAVEHICULAR MOBILITY UNIT ROUTINE GONE EXTREMELY NOT COOL YO", width/2, height/4*1.3f);
  text("EARTH ART BY MICHAEL J PIERCE - ISOMETRICROBOT.COM", width/2, height/4*2.2f);
  text("CARDS_UI BY LUCAS CASSIANO - CASSIANO@MIT.EDU", width/2, height/4*2.3f);
  textSize(width/25);
}
...
public void saveData() {
  if (minutes>=minsHS&&seconds>secsHS||minutes>minsHS||minsHS==0&&secsHS==0) {
    String save = "$"+str(minutes)+"$"+str(seconds);
    //String[] saved = split(save, '$');
    //saveStrings("data.txt", saved);    
    PrintWriter  output = createWriter(directory + "/" + "atextFilet.txt"); 
    output.println(save);
    output.flush();
    output.close();
  }
...
class Player {
  ...
  public void update() {
    ...
    //movement
    if ((gameState!=0&&gameState<2)&&mousePressed&&health>0&&(invincibleTimer==0) || mouseY<height-width/5&&mousePressed&&health>0&&(invincibleTimer==0)) {
